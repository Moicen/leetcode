...
const rob = (root) => {
    ...
    const recur = (node) => {
        ...
        let cache = dp.get(node);
        if (cache) return cache;
        let ll = recur((left || {}).left), lr = recur((left || {}).right);
        let rl = recur((right || {}).left), rr = recur((right || {}).right);
        let sum = Math.max(val + ll + lr + rl + rr, recur(left) + recur(right));
        dp.set(node, sum);
        return sum;
    };
...
const printTree = (root) => {
    ...
    const place = (node, depth, l, r) => {
        if (!node) return;
        let { val, left, right } = node;
        let pos = Math.pow(2, m - (depth + 1)) - 1;
        let index = pos + l;
        result[depth][index] = val.toString();
        place(left, depth + 1, l, index - 1);
        place(right, depth + 1, index + 1, r);
    };
...
const path_sum = (root, sum) => {
    ...
    const dfs = (node, path, remain) => {
        ...
        remain -= val;
        let next = [...path, val];
        if (!remain && !left && !right) paths.push([...next]);
        dfs(left, [...next], remain);
        dfs(right, [...next], remain);
...
const zigzagLevelOrder = (root) => {
    ...
    let queue = [root], nodes = [], reverse = false;
    while (queue.length) {
        let len = queue.length, row = [];
        while (len--) {
            let node = queue.shift();
